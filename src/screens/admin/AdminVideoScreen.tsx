...
import { Search, Plus, Edit3, Trash2, Eye, Video, CheckCircle2, Clock } from 'lucide-react';
...
interface AdminVideoScreenProps {
  onNavigate: (path: string) => void;
}

export const AdminVideoScreen: React.FC<AdminVideoScreenProps> = ({ onNavigate }) => {
  const { data, deleteVideo } = useData();
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState<'all' | 'published' | 'draft'>('all');
  const [itemToDelete, setItemToDelete] = useState<VideoItem | null>(null);
  ...
  }, [data.videos, searchQuery, statusFilter]);

  const handleDeleteConfirm = () => {
    if (itemToDelete) {
      deleteVideo(itemToDelete.id);
      showToast(`“${itemToDelete.title}” video delete ho gaya`);
  ...
  return (
    <PageContainer>
      <AdminPageHeader
        title="वीडियो प्रबंधन (Video Management)"
        subtitle="व्याख्यान, सत्संग और परिचर्चा वीडियो प्रबंधित करें"
        badge={`${data.videos.length} Total`}
        onBack={() => onNavigate('/admin')}
        backLabel="Admin Dashboard"
        primaryAction={{
          label: 'नया वीडियो जोड़ें',
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="वीडियो खोजें (Title, Speaker, Topic)..."
      ...
      {filteredItems.length === 0 ? (
        <div className="p-8 text-center bg-white dark:bg-[#1E293B] rounded-2xl border border-[#E8E5DF] dark:border-[#334155]">
          <Video size={32} className="mx-auto text-[#8C96A3] mb-2" />
          <h3 className="text-[15px] font-bold text-[#1F2421] dark:text-white">कोई वीडियो नहीं मिला</h3>
          <p className="text-[12.5px] text-[#5C6773] dark:text-gray-400 mt-1 mb-4">
            {searchQuery ? 'खोज के अनुसार कोई वीडियो उपलब्ध नहीं है।' : 'अभी तक कोई वीडियो नहीं जोड़ा गया है।'}
          </p>
          <button
            type="button"
            onClick={() => onNavigate('/admin/video/new')}
          ...
          {filteredItems.map((item) => (
            ...
            >
              <div className="flex items-start gap-3 flex-1 min-w-0">
                {/* Video Thumbnail */}
                <div className="w-20 h-14 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0 border border-[#E8E5DF] dark:border-[#334155]">
                  {item.thumbnailUrl ? (
                    <img src={item.thumbnailUrl} alt={item.title} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[#8C96A3]">
                      <Video size={20} />
                    </div>
                  )}
                </div>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[11.5px] text-[#8C96A3]">⏳ {item.duration}</span>
                  </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                {item.status === 'published' && (
                  <button
                    type="button"
                    onClick={() => onNavigate(`/video/${item.id}`)}
                ...
                <button
                  type="button"
                  onClick={() => onNavigate(`/admin/video/${item.id}/edit`)}
